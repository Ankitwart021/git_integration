...
const  WORKFLOW_BASE_URL  = apiConfig.WORKFLOW_BASE_URL;

export const api = {
  async fetchWorkflowInbox(workflowIds: string[], roles: string[]): Promise<{ data: any}> {
    try {
      const response = await fetch(`${WORKFLOW_BASE_URL}/workflow_execution_context`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include", // Send cookies with request
        body: JSON.stringify({ workflow_ids: workflowIds, roles: roles }),
      });
...
async getWorkflowHistory(workflowId: string, executionId: string): Promise<any[]> {
  ...
  async startWorkflow(workflowDefId: string, userId: string,role: string[], initialData: Record<string, any>): Promise<any> {
    try {
      const response = await fetch(`${WORKFLOW_BASE_URL}/invoke`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include", // Send cookies with request
        body: JSON.stringify({
  ...
  async resumeWorkflow(workflowId: string, userId: string, input:any, role: string,thread_id: string): Promise<any> {
    try {
      const response = await fetch(`${WORKFLOW_BASE_URL}/resume`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include", // Send cookies with request
        body: JSON.stringify({
