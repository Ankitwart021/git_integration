...
const WorkflowListExecutions: React.FC = () => {
  ...
  const { data, isLoading , error} = useWorkflowInbox(workflowIds, roles);

useEffect(() => {
  if (!data?.data || !selectedWorkflow?.id) {
    setExecutions([]);
    return;
  }

  const workflowContext = data.data.find(
    (item: any) => item.workflow.id === selectedWorkflow.id
  );

  setExecutions(workflowContext?.executions || []);
}, [data, selectedWorkflow]);


const handleSelectExecution = (execution: WorkflowExecution) => {
  setSelectedExecution(execution);
  navigate(`/workflow/workflow_form/${execution.id}`);
};
            ...
            <div className="col-lg-5 col-md-7">
              <div className="card border-0 rounded-4 shadow-lg overflow-hidden">
                ...
                <div className="card-body text-center p-5">
                  ...
                  <p className="text-muted mb-4 lead">{error?.message}</p>
                  <button 
                ...
                <div className="d-flex align-items-center gap-3">
                  ...
                  <div>
                    <h2 className="fw-bold mb-0 text-capitalize" style={{ fontSize: '1.75rem' }}>
                      {currentUser?.resource_access?.client_id?.roles[0]?.replace(/_/g, ' ') || "N/A"}
                    </h2>
                ...
                <div className="d-flex align-items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mb-0 text-muted small">
                      Showing {executions.length} of {executions.length} total runs
                    </p>
                  </div>
                </div>
            ...
            <div className="card-footer border-0 py-3" style={{ background: '#f8f9fa' }}>
              <div className="d-flex align-items-center justify-content-between text-muted small">
                <div className="d-flex align-items-center gap-2">
                  ...
                  <span>
                    Displaying <strong>{executions.length}</strong> execution{executions.length !== 1 ? 's' : ''}
                  </span>
                </div>
                <div className="d-flex align-items-center gap-2">
