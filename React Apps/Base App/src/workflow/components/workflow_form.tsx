...
const ShowForm = () => {
  ...
  const { selectedWorkflow, selectedExecution, availableRoles } = useWorkflowStore();
  console.log("Selected execution in Form:", selectedExecution);
  ...
  // useEffect(() => { console.log("Current Node", currentNode) }, [currentNode])
    ...
    if (data && data[0]?.next?.[0]) {
      ...
      for (const node of selectedWorkflow.workflow_spec.nodes) {
        ...
        if (node.id === data[0].next[0]) {
          ...
          setCurrentNode(node);
          const formInput = (node as any).inputs?.find((input: any) =>
            input.type === "dict" && input.name === "form" && input.fields && input.fields.length > 0
          );
          const initialData = formInput?.fields.reduce((acc: any, f: any): any => {
...
const renderFormField = (field: any) => {
    ...
    case "textarea":
      return (
        <textarea
          ...
          style={{ minHeight: "120px", resize: "vertical", transition: "all 0.2s ease" }}
        ...
        <div>
          ...
          {formData[field.name] && (
            <div className="mt-3 p-3 bg-success bg-opacity-10 border border-success rounded-3">
              ...
              <span className="text-success fw-semibold">File uploaded successfully</span>
    ...
    resumeWorkflowMutation.mutate(
      {
        ...
        role: currentUser?.resource_access?.[currentUser?.azp]?.roles,
  ...
  const latestHistory = data
    ? [...data].reverse().filter((d: any) => d.values && Object.keys(d.values).length > 0)
    : [];

  const handle_sort_and_remove_duplicates = (latestHistory: any, orderedNodes: string[]) => {
  ...
  const finalHistory = handle_sort_and_remove_duplicates(latestHistory, orderedNodes);
  const sortedFinalHistory = [...finalHistory].sort(
    (a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()
  );

  return (
    <div className="container-fluid py-4 p-5" style={{ background: "linear-gradient(135deg, #f5f7fa 0%, #e4e9f2 100%)", minHeight: "100vh" }}>
      ...
      <div className="row">
        ...
        <div className="col-lg-7 col-xl-8 mb-4">
            ...
            >
              <div className="d-flex justify-content-between align-items-start mb-3">
                ...
                <span
                  ...
                  style={{ fontSize: "0.85rem", fontWeight: "600", borderRadius: "20px" }}
                >
              ...
              <div className="border-top border-white border-opacity-25 pt-3 mt-3">
                <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
                  <div className="d-flex align-items-center gap-2">
                    ...
                    <div>
                      <small className="d-block opacity-75" style={{ fontSize: "0.75rem" }}>Logged in as</small>
                      <strong style={{ fontSize: "0.95rem" }}>{currentUser ? currentUser.name : "User"}</strong>
                    </div>
                  </div>
                  <div className="d-flex align-items-center gap-2">
                    <span
                      className="badge bg-dark bg-opacity-20 px-3 py-2"
                      style={{ fontSize: "0.85rem", fontWeight: "500", borderRadius: "20px" }}
                    >
                      {currentUser?.role}
                    </span>
                    <select
                      ...
                      onChange={(e) =>
                        currentUser && setCurrentUser({ ...currentUser, role: e.target.value })
                      }
                      style={{
                        width: "auto",
                        minWidth: "120px",
                  ...
                  {currentNode.inputs?.map((input: any, inputIdx: number) => (
                    input.fields?.map((field: any, fieldIdx: number) => (
                      <div
                        ...
                        style={{
                          background: fieldIdx % 2 === 0 ? "#f8f9fa" : "transparent",
                          transition: "all 0.2s ease"
                        }}
                      >
                        <label className="form-label fw-semibold d-flex align-items-center mb-3" style={{ color: "#2d3748", fontSize: "0.95rem" }}>
                          <span
                          ...
                          {field.name.replace(/_/g, " ").replace(/\b\w/g, (c: any) => c.toUpperCase())}

                          {field.required && canWriteField(field) && (
                            <span className="text-danger ms-1" style={{ fontSize: "1.1rem" }}>*</span>
                          )}
                        </label>
          ...
          {currentUser?.role === "admin" && (
            ...
            >
              <h5 className="alert-heading fw-bold d-flex align-items-center" style={{ color: "#8b4513" }}>
        ...
        <div className="col-lg-5 col-xl-4">
          <div className="position-sticky" style={{ top: "20px" }}>
            <div className="mb-4 d-flex align-items-center gap-3">
              ...
              <div>
                <h4 className="fw-bold mb-0" style={{ color: "#2d3748" }}>Workflow History</h4>
            ...
            >
              ...
              {sortedFinalHistory.reverse().map((item, index) => {
                ...
                const stepFormData = values?.form|| values.status || values || {};
                    ...
                    >
                      <div className="d-flex justify-content-between align-items-center">
                        <div className="d-flex align-items-center gap-3">
                          ...
                          <div>
                            <strong style={{ color: "#2d3748", fontSize: "0.95rem" }}>{node_id}</strong>
                            ...
                            return (
                              <div className="ms-3 mt-2">
                                {Object.entries(val).map(([k, v]) => (
                                  <div key={k} className="mb-2 p-2 rounded-3" style={{ background: "#f8f9fa" }}>
                            ...
                            const handleDownload = async () => {
                              try {
                                const response = await fetch(
                                  `http://localhost:8085/api/documents/${fileId}/download?dmsRole=admin&userId=${currentUser ? currentUser.id : "1234"
                                  }`
                                );
                          ...
                          <div key={key} className="mb-3 pb-3 border-bottom">
                            <div className="row align-items-start">
                              <div className="col-md-4">
                                <span className="text-secondary fw-semibold text-capitalize d-block" style={{ fontSize: "0.85rem" }}>
                                  {key.replace(/_/g, " ")}
                                </span>
                              </div>
